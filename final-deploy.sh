...
echo "🚀 Deploy final - Colocando tudo no GitHub!"
echo ""

# Cria os scripts localmente

echo "📝 Criando install.sh..."
cat > install.sh << 'INSTALL_SH'
#!/bin/bash
set -e
...
chmod +x install.sh
echo "✅ install.sh criado"

echo ""
echo "📤 Commitando e pushando..."
git add install.sh final-deploy.sh index.html
git commit -m "🚀 Add installation system: auto-install scripts + dashboard" 2>/dev/null || echo "⚠️ Nenhuma mudança"
git push

echo ""
echo "✅ DEPLOY COMPLETO!"
echo ""
echo "Site: https://teste0102.github.io/Skills/"
